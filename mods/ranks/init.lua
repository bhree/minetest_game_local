...
minetest.register_on_joinplayer(function(player)
	if ranks.get_rank(player) then
	    ...
	    if global_exchange:get_balance(player:get_player_name()) >= 100 then
			ranks.set_rank(player, "private")
		else
...
local kick = minetest.registered_chatcommands["kick"].func
-- [override] /kick	
minetest.registered_chatcommands["kick"].func = function(name, param)
		local kickerrank = ranks.get_rank(name)
		local tokick, reason = param:match("([^ ]+) (.+)")
		tokick = tokick or param
		local kickedrank = ranks.get_rank(tokick)
		if kickerrank == kickedrank then
			return false, "Can't kick equal rank of "
		end
-- general
		if kickerrank == "general" then
			if not core.kick_player(tokick, reason) then
				return false, "Failed to kick player " .. tokick
			end
-- lt gen		
		elseif kickerrank == "leutenantgen" then
			if kickedrank == "general" then
				return false, "Can't kick higher rank"
			end
			if not core.kick_player(tokick, reason) then
				return false, "Failed to kick player " .. tokick
			end	
-- maj gen
		elseif kickerrank == "majorgen" then
			if kickedrank == "leutenantgen" or kickedrank == "general" then
				return false, "Can't kick higher rank"
			end
			if not core.kick_player(tokick, reason) then
				return false, "Failed to kick player " .. tokick
...
-- bgd gen
		elseif kickerrank == "brigadiergen" then
			if kickedrank == "majorgen" or kickedrank == "leutenantgen" or kickedrank == "general" then
				return false, "Can't kick higher rank"
			end
			if not core.kick_player(tokick, reason) then
				return false, "Failed to kick player " .. tokick
...
-- colonel
		elseif kickerrank == "colonel" then
			if kickedrank == "brigadiergen" or kickedrank == "majorgen" or kickedrank == "leutenantgen" or kickedrank == "general" then
				return false, "Can't kick higher rank"
			end
			if not core.kick_player(tokick, reason) then
				return false, "Failed to kick player " .. tokick
...
-- captain
		elseif kickerrank == "captain" then
			if kickedrank == "colonel" or kickedrank == "brigadiergen" or kickedrank == "majorgen" or kickedrank == "leutenantgen" or kickedrank == "general" then
				return false, "Can't kick higher rank"
			end
			if not core.kick_player(tokick, reason) then
				return false, "Failed to kick player " .. tokick
...
-- sergeant
		elseif kickerrank == "sergeant" then
			if kickedrank == "captain" or kickedrank == "colonel" or kickedrank == "brigadiergen" or kickedrank == "majorgen" or kickedrank == "leutenantgen" or kickedrank == "general" then
				return false, "Can't kick higher rank"
			end
			if not core.kick_player(tokick, reason) then
				return false, "Failed to kick player " .. tokick
...
-- corporal
		elseif kickerrank == "corporal" then
			if kickedrank == "sergeant" or kickedrank == "captain" or kickedrank == "colonel" or kickedrank == "brigadiergen" or kickedrank == "majorgen" or kickedrank == "leutenantgen" or kickedrank == "general" then
				return false, "Can't kick higher rank"
			end
			if not core.kick_player(tokick, reason) then
				return false, "Failed to kick player " .. tokick
