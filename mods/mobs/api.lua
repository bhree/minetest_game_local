...
local mob_is_solid_block = function(pos)
...
local mob_staticdata = function(self)
	...
	and self.lifetimer < 20000 then
		...
		-- self.object:remove()
		self.lifetimer = self.lifetimer - 5
...
local mob_step = function(self, dtime)
    ...
    if mob_is_solid_block(pos) then -- I could probably make an anticheat mod with this kind of thing, if I cared.
		local apos = vector.subtract(pos, vector.round(pos))
		local changed
		if apos.x > 0.33 and not mob_is_solid_block({x=pos.x+0.5,y=pos.y,z=pos.z}) then
			pos.x = pos.x + 0.5
			changed = true
		end
		if apos.z > 0.33 and not mob_is_solid_block({x=pos.x,y=pos.y,z=pos.z+0.5}) then
			pos.z = pos.z + 0.5
			changed = true
		end
		if apos.x < -0.33 and not mob_is_solid_block({x=pos.x-0.5,y=pos.y,z=pos.z}) then
