...
minimal_anticheat.secondary_check_cheater_on_coal = function(player, pos)
    if player and player:is_player() and player:get_hp() > 0 and
        ...
        irc.say("Player " .. name .. " seems noclipping")
        minetest.chat_send_player(name, "You are moving past nodes");
	...
	for _,player in pairs(minetest.get_connected_players()) do
		if math.random(1, 100) > 50 and player and player:is_player() then
			...
			if player:get_hp() > 0 and pos.y > 10 then	--check on air
				...
				--]]
                    local name = player:get_player_name()
                    irc.say("Player " .. name .. " seems flying")
                    minetest.chat_send_player(name, "You are standing on nothing");
...
minimal_anticheat.check_cheater_by_engine = function (player, cheat)
    if player:is_player() then
        ...
        if cheat.type == "dug_too_fast" then
            ...
            --]]
                irc.say("Player " .. name .. " seems dig cheating")
                minetest.chat_send_player(name, "You are digging too fast");
        ...
        elseif cheat.type == "interacted_too_far" then
            ...
            if math.random(1, 100) > 80 then
                ...
                --]]
                    irc.say("Player " .. name .. " seems interacted too far")
                    minetest.chat_send_player(name, "You are interacting too far");
        ...
        elseif cheat.type == "moved_too_fast" then
            ...
            if math.random(1, 100) > 90 then
                ...
                --]]
                    irc.say("Player " .. name .. " seems too fast")
                    minetest.chat_send_player(name, "You are moving too fast");
