aliveai.rnd_talk_to=function(self,ob)
	...
	if bot or player then
		...
		elseif r==11 then
			aliveai.say(self,"go to GATHER you'll be safe.")
		end
	end
end
...
aliveai.on_spoken_to=function(self,name,speaker,msg)
	...
	aliveai.sub_response(self,msg,5,{"go back to your SPAWN"},{"where is it?", "where the spawn?", "what spawn?","what is that?"})
	aliveai.sub_response(self,msg,5,{{"tpr to me"},{"teleport to me"},{"tp me to"},{"teleport me to"}},{"im not a travelnet!", "admin, " .. speaker .. " want teleport"})
	...
	elseif aliveai.expected_response(self,speaker,msg,"this") then
		aliveai.say(self,aliveai.rndkey({"ok","coming","right"}))
		msg="come"
	end
		...
		aliveai.find(msg,{"who","are","you"},self,"an npc")
		...
		if aliveai.find(msg,{"how are you"}) or aliveai.find(msg,{"how do you feel"}) or aliveai.find(msg,{"whats up?"}) then
			...
			elseif self.mood<1 then aliveai.say(self,"nothing")
