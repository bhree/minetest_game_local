...
radiant_damage.register_radiant_damage("mese", {
	...
	inverse_square_falloff = true,
	emitted_by = {["default:stone_with_mese"] = radiant_damage.config.mese_damage, ["radiant_damage:radiomese"] = radiant_damage.config.mese_damage * 9},
	attenuated_by = {["group:stone"] = 0.5, ["group:mese_radiation_shield"] = 0.1, ["group:mese_radiation_amplifier"] = 4},
	default_attenuation = 0.9,
})
end

radiant_damage.register_radiant_damage("evil", {
	interval = 1,
	emitted_by = {["nssb:mornen"] = radiant_damage.config.lava_damage, ["nssb:mornen_flowing"] = radiant_damage.config.lava_damage},
	inverse_square_falloff = true,
	default_attenuation = 0, -- heat is blocked by anything.
})

minetest.register_node("radiant_damage:radiomese", {
	description = "Radioactive Mese Block",
    ...
    on_place = function(itemstack, placer, pointed_thing)
		if pointed_thing.type == "node" and not minetest.is_protected(pointed_thing.above, placer:get_player_name()) then
			local n = minetest.env:get_node(pointed_thing.above)
			if n.name ~= ""  and n.name == "air" then
				minetest.env:set_node(pointed_thing.above, {name="default:mese"})
			end
		end
	end,
