...
local progressive_mode = M.settings:get_bool("craft_guide_progressive_mode")
local sfinv_only = M.settings:get_bool("craft_guide_sfinv_only") and rawget(_G, "sfinv")
...
local S = M.get_translator("craft_guide")
...
function craft_guide.register_craft_type(name, def)
	local func = "craft_guide." .. __func() .. "(): "
...
function craft_guide.register_craft(def)
	local func = "craft_guide." .. __func() .. "(): "
...
function craft_guide.add_recipe_filter(name, f)
	local func = "craft_guide." .. __func() .. "(): "
...
function craft_guide.remove_recipe_filter(name)
	recipe_filters[name] = nil
end

function craft_guide.set_recipe_filter(name, f)
	local func = "craft_guide." .. __func() .. "(): "
	assert(is_str(name), func .. "filter name missing")
	assert(is_func(f), func .. "filter function missing")
...
function craft_guide.get_recipe_filters()
...
function craft_guide.add_search_filter(name, f)
	local func = "craft_guide." .. __func() .. "(): "
	assert(is_str(name), func .. "filter name missing")
	assert(is_func(f), func .. "filter function missing")
...
function craft_guide.remove_search_filter(name)
	search_filters[name] = nil
end

function craft_guide.get_search_filters()
	return search_filters
end

local formspec_elements = {}

function craft_guide.add_formspec_element(name, def)
	local func = "craft_guide." .. __func() .. "(): "
...
function craft_guide.remove_formspec_element(name)
	formspec_elements[name] = nil
end

function craft_guide.get_formspec_elements()
...
local function groups_to_item(groups)
	if #groups == 1 then
		...
		local stereotypes = craft_guide.group_stereotypes
...
local function get_recipe_fs(data, iY)
	...
	if custom_recipe or shapeless or recipe.type == "cooking" then
		...
		if not custom_recipe then
			icon = fmt("craft_guide_%s.png^[resize:16x16", icon)
		end
...
local function make_formspec(name)
	...
	if not sfinv_only then
		...
		fs[#fs + 1] = [[
			no_prepend[]
			background[1,1;1,1;craft_guide_bg.png;true]
		]]
		...
		fs[#fs + 1] = fmt([[
			image_button[%f,0.12;0.8,0.8;craft_guide_zoomin_icon.png;size_inc;]
			image_button[%f,0.12;0.8,0.8;craft_guide_zoomout_icon.png;size_dec;] ]],
			data.iX * 0.47,
			data.iX * 0.47 + 0.6)
	end

	fs[#fs + 1] = [[
		image_button[2.4,0.12;0.8,0.8;craft_guide_search_icon.png;search;]
		image_button[3.05,0.12;0.8,0.8;craft_guide_clear_icon.png;clear;]
	...
	fs[#fs + 1] = fmt([[
		image_button[%f,0.12;0.8,0.8;craft_guide_prev_icon.png;prev;]
		image_button[%f,0.12;0.8,0.8;craft_guide_next_icon.png;next;] ]],
...
local show_fs = function(player, name)
	if sfinv_only then
		...
		show_formspec(name, "craft_guide", make_formspec(name))
	end
end

craft_guide.add_search_filter("groups", function(item, groups)
...
if sfinv_only then
	sfinv.register_page("craft_guide:craft_guide", {
		title = S("Craft Guide"),
	...
	M.register_craftitem("craft_guide:book", {
		description = S("Crafting Guide"),
		inventory_image = "craft_guide_book.png",
		wield_image = "craft_guide_book.png",
		stack_max = 1,
		groups = {book = 1},
	...
	M.register_node("craft_guide:sign", {
		description = S("Crafting Guide Sign"),
		drawtype = "nodebox",
		tiles = {"craft_guide_sign.png"},
		inventory_image = "craft_guide_sign_inv.png",
		wield_image = "craft_guide_sign_inv.png",
		paramtype = "light",
...
-- craft pc
M.register_node("craft_guide:lcd_pc", {
	...
	M.register_craft({
		output = "craft_guide:book",
		type = 'shapeless',
	recipe = {
		'default:mese_crystal', 'default:diamond', 'default:steel_ingot',
		'default:copper_ingot', 'default:tin_ingot', 'default:coal_lump',
		'craftguide:book', 'group:stone'
	}
	})

	M.register_craft({
		type = "fuel",
		recipe = "craft_guide:book",
		burntime = 3
	})

	M.register_craft({
		output = "craft_guide:sign",
		type = 'shapeless',
	recipe = {
		'default:mese_crystal', 'default:diamond', 'default:steel_ingot',
		'default:copper_ingot', 'default:tin_ingot', 'default:coal_lump',
		'craftguide:sign', 'group:stone'
	}
	})

	M.register_craft({
		type = "fuel",
		recipe = "craft_guide:sign",
		burntime = 10
	})
...
if progressive_mode then
	...
	craft_guide.add_recipe_filter("Default progressive filter", progressive_filter)
...
M.register_chatcommand("craft", {
	...
	func = function(name)
		...
		local red = colorize("red", "[craft_guide] ")
...
function craft_guide.show(name, item, show_usages)
	local func = "craft_guide." .. __func() .. "(): "
