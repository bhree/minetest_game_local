...
dmobs.dragon.step_custom_ice = function(self, dtime)
	if self.driver then
...
dmobs.dragon.step_custom_poison = function(self, dtime)
	if self.driver then
...
dmobs.dragon.step_custom_lightning = function(self, dtime)
	if self.driver then
		object_fly(self, dtime, 10, true, "dmobs:lightning_plyr", "walk", "stand")
		if self.state == "attack" then
			self.state = nil
		end
		return false
	...
	if self.tamed and self.owner == clicker:get_player_name() then
		...
		elseif not self.driver then
			if clicker:get_wielded_item():get_name() == "mobs:saddle" then
				...
				mobs:capture_mob(self, clicker, 0, 0, 100, false, nil)
			end
		end
	end
end
...
dmobs.dragon.do_custom_ice = function(self, dtime)
	if self.driver then
		object_fly(self, dtime, 10, true, "dmobs:ice_plyr", "walk", "stand")
...
dmobs.dragon.do_custom_poison = function(self, dtime)
	if self.driver then
		object_fly(self, dtime, 10, true, "dmobs:poison_plyr", "walk", "stand")
...
dmobs.dragon.do_custom_lightning = function(self, dtime)
	if self.driver then
		object_fly(self, dtime, 10, true, "dmobs:lightning_plyr", "walk", "stand")
