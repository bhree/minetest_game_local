...
hardtorch.register_tool = function(torchname, def)
	...
	minetest.override_item(torchname, {
		on_use = function(itemstack, user, pointed_thing)
			if itemstack:get_name() ~= torchname then
print("0")
				return
			end
...
minetest.register_on_leaveplayer(function(user)
	local temp_player_name = user:get_player_name()
	local temp_loop = hardtorch.em_loop[temp_player_name]
	if temp_loop then
		if temp_loop.fuel and temp_loop.fuel.name then
			hardtorch.apagar_tocha(user, temp_loop.fuel.name)
		end
		hardtorch.em_loop[temp_player_name] = nil
	end
end)