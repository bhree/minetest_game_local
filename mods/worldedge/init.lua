...
local edge = tonumber(minetest.settings:get("world_edge")) or 21000
...
minetest.register_globalstep(function(dtime)   
	...
	for i, player in ipairs(players) do
		...
		if not waiting_list[name] then
			...
			local newpos = nil
			if pos.x >= edge and pos.x <= edge + 1000 then
				newpos = {x = -newedge, y = 10, z = pos.z}
			elseif pos.x <= -edge and pos.x >= -edge - 1000 then
				newpos = {x = newedge, y = 10, z = pos.z}
			end
			 
			if pos.z >= edge and pos.z <= edge + 1000 then
				newpos = {x = pos.x, y = 10, z = -newedge}
			elseif pos.z <= -edge and pos.z >= -edge - 1000 then
				newpos = {x = pos.x, y = 10, z = newedge}
			end
