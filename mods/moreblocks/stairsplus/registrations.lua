...
for _, name in pairs(moreores_nodes) do
	local mod = "moreores"
	local nodename = mod .. ":" .. name
	local ndef = table.copy(minetest.registered_nodes[nodename])
	ndef.sunlight_propagates = true
...
if minetest.get_modpath("nssb") then
	local nssb_nodes = {"morentir"}
	for _, name in pairs(nssb_nodes) do
		local mod = "nssb"
		local nodename = mod .. ":" .. name
		local ndef = table.copy(minetest.registered_nodes[nodename])
		ndef.sunlight_propagates = true
...
-- space registrations
if minetest.get_modpath("space") then
	local space_nodes = {"stone", "moon_stone"}
	for _, name in pairs(space_nodes) do
		local mod = "space"
		local nodename = mod .. ":" .. name
		local ndef = table.copy(minetest.registered_nodes[nodename])
		ndef.sunlight_propagates = true
